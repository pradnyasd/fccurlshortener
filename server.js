...
var mongoURL = 'mongodb://akshatamw:akshata2607@ds153730.mlab.com:53730/amsdb';
               ...
               if (regex.test(req.params.url)) {
                  collection.count().then(function(number) {
                     var newElement = {
                        ...
                        short_url: "https://steady-shield.glitch.me/" + (number + 1)
                     }
   ...
   MongoClient.connect(mongoURL, function(err, db) {
      if (err) {
         ...
         var query = function(db, callback) {
            collection.findOne({
               "short_url": "https://steady-shield.glitch.me/" + req.params.shortid
            }, {
               original_url: 1,
               _id: 0
            }, function(err, answer) {
